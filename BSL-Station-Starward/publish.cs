...
if (string.Equals(command, "res", StringComparison.OrdinalIgnoreCase))
{
    ...
    if (!noBuild)
    {
        ...
        File.Move("publish/pub_res/BSL-Station.Setup.exe", $"publish/pub_res/BSL-Station.Setup_x64_{tag}.exe", true);

        await Process.Start("dotnet", $"publish src/Starward.Setup -o publish/pub_res/ -r win-arm64 -p:Version={tag}").EnsureExitSuccessAsync();
        File.Move("publish/pub_res/BSL-Station.Setup.exe", $"publish/pub_res/BSL-Station.Setup_arm64_{tag}.exe", true);

        await Process.Start(msbuildPath, $"""
        ...
        File.Move("publish/pub_res/BSL-Station.exe", $"publish/pub_res/BSL-Station_x64_{tag}.exe", true);

        await Process.Start(msbuildPath, $"""
        src/Starward.Launcher -property:Configuration=Release;Platform=arm64;Version={tag};OutDir={Path.GetFullPath("publish/pub_res/")}
        """).EnsureExitSuccessAsync();
        File.Move("publish/pub_res/BSL-Station.exe", $"publish/pub_res/BSL-Station_arm64_{tag}.exe", true);

        await Process.Start("upx", $"publish/pub_res/BSL-Station.Setup_x64_{tag}.exe").EnsureExitSuccessAsync();
    }
    ...
    var buildRes = new BuildResource
    {
        ...
        {
            FileName = "BSL-Station.Setup.exe",
            Size = new FileInfo($"publish/pub_res/BSL-Station.Setup_x64_{tag}.exe").Length,
            Hash = Convert.ToHexStringLower(SHA256.HashData(File.ReadAllBytes($"publish/pub_res/BSL-Station.Setup_x64_{tag}.exe"))),
            Url = $"{UrlPrefix}/pub_res/BSL-Station.Setup_x64_{tag}.exe",
        },
        SetupArm64 = new ReleaseSetup
        {
            FileName = "BSL-Station.Setup.exe",
            Size = new FileInfo($"publish/pub_res/BSL-Station.Setup_arm64_{tag}.exe").Length,
            Hash = Convert.ToHexStringLower(SHA256.HashData(File.ReadAllBytes($"publish/pub_res/BSL-Station.Setup_arm64_{tag}.exe"))),
            Url = $"{UrlPrefix}/pub_res/BSL-Station.Setup_arm64_{tag}.exe",
        },
        LauncherX64 = new ReleaseSetup
        {
            FileName = "BSL-Station.exe",
            Size = new FileInfo($"publish/pub_res/BSL-Station_x64_{tag}.exe").Length,
            Hash = Convert.ToHexStringLower(SHA256.HashData(File.ReadAllBytes($"publish/pub_res/BSL-Station_x64_{tag}.exe"))),
            Url = $"{UrlPrefix}/pub_res/BSL-Station_x64_{tag}.exe",
        },
        LauncherArm64 = new ReleaseSetup
        {
            FileName = "BSL-Station.exe",
            Size = new FileInfo($"publish/pub_res/BSL-Station_arm64_{tag}.exe").Length,
            Hash = Convert.ToHexStringLower(SHA256.HashData(File.ReadAllBytes($"publish/pub_res/BSL-Station_arm64_{tag}.exe"))),
            Url = $"{UrlPrefix}/pub_res/BSL-Station_arm64_{tag}.exe",
        }
    };
...
if (doCompile)
{
    foreach (var arch in targetArchitectures)
    {
        ...
        await Process.Start(msbuildPath, $"""
        src/Starward.Launcher -property:Configuration=Release;Platform={archName};Version={version};OutDir={Path.GetFullPath($"{archPath}/Starward/")}
        """).EnsureExitSuccessAsync();
...
async Task CreatePackageAsync(string version, Architecture arch, InstallType type, ReleaseInfo release_info, bool enableDiff, bool doPackage, bool doManifest)
{
    ...
    if (doPackage)
    {
        ...
        if (type is InstallType.Setup)
        {
            ...
            File.Move($"publish/{arch.ToLower()}-setup/BSL-Station.Setup.exe", $"publish/release/package/BSL-Station_Setup_{version}_{arch.ToLower()}.exe", true);
            File.Delete("src/Starward.Setup/Assets/Starward.7z");
        }
        else
        {
            Console.WriteLine("Compressing portable package...");
            string launcherPath = Path.Join(rootPath, "BSL-Station.exe");
            if (!File.Exists(launcherPath))
            {
                throw new FileNotFoundException("BSL-Station launcher not found.", launcherPath);
            }
            compressor.CompressDirectory(Path.GetDirectoryName(rootPath.TrimEnd('/', '\\'))!, $"publish/release/package/BSL-Station_Portable_{version}_{arch.ToLower()}.7z");
        }
    ...
    string packageFileName = type is InstallType.Setup ? $"BSL-Station_Setup_{version}_{arch.ToLower()}.exe" : $"BSL-Station_Portable_{version}_{arch.ToLower()}.7z";
...
{
    string? envPath = Environment.GetEnvironmentVariable("MSBUILD_EXE_PATH");
