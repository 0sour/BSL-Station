...
int wmain(int argc, wchar_t* argv[])
{
	...
	if (std::filesystem::exists(version_file))
	{
		...
		if (ini.ParseError() == 0)
		{
			...
			{
				std::filesystem::path target_exe = base_folder / ("app-" + version_text) / "BSL-Station.exe";
	...
	if (run_exe.empty())
	{
		...
		for (const std::filesystem::directory_entry& folder : std::filesystem::directory_iterator(base_folder, std::filesystem::directory_options::skip_permission_denied))
		{
			if (folder.is_directory())
			{
				...
				std::filesystem::path exe = folder.path() / L"BSL-Station.exe";
	...
	{
		SetProcessDPIAware();
		int ok = MessageBox(NULL, L"BSL-Station files not found.", L"BSL-Station", MB_ICONWARNING | MB_OK);
		if (ok == IDOK)
		{
		}
	}
