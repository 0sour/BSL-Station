...
internal class GameInstallService
{
    ...
    private static readonly JsonSerializerOptions NativeHistoryJsonOptions = new()
    {
        PropertyNameCaseInsensitive = true,
        WriteIndented = true,
    };

    private static string NativeHistoryStorePath => Path.Combine(AppConfig.CacheFolder, "BSL", "native_install_history.json");
    ...
    public IReadOnlyList<GameInstallHistoryItem> GetGameInstallHistory()
    {
        try
        {
            if (!File.Exists(NativeHistoryStorePath))
            {
                return [];
            }

            string json = File.ReadAllText(NativeHistoryStorePath, Encoding.UTF8);
            return JsonSerializer.Deserialize<List<GameInstallHistoryItem>>(json, NativeHistoryJsonOptions) ?? [];
        }
        catch
        {
            return [];
    ...
    public void ClearGameInstallHistory()
    {
        try
        {
            if (File.Exists(NativeHistoryStorePath))
            {
                File.Delete(NativeHistoryStorePath);
            }
        }
        catch
        {
    ...
    private void AppendNativeHistory(GameInstallContext task)
    {
        try
        {
            List<GameInstallHistoryItem> items = GetGameInstallHistory().ToList();
            items.RemoveAll(x => x.Id == GameInstallHistoryItem.CreateId(task.GameId, task.Operation, task.InstallPath));
            items.Insert(0, GameInstallHistoryItem.FromTask(task));
            if (items.Count > 30)
            {
                items.RemoveRange(30, items.Count - 30);
            }

            string? directory = Path.GetDirectoryName(NativeHistoryStorePath);
            if (!string.IsNullOrWhiteSpace(directory))
            {
                Directory.CreateDirectory(directory);
            }

            string json = JsonSerializer.Serialize(items, NativeHistoryJsonOptions);
            File.WriteAllText(NativeHistoryStorePath, json, Encoding.UTF8);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Append native install history failed.");
        }
    }
...
public sealed class GameInstallHistoryItem
{
    public string Id { get; set; } = string.Empty;

    public string GameBizValue { get; set; } = string.Empty;
    ...
    public long DownloadFinishBytes { get; set; }

    public long WriteTotalBytes { get; set; }

    public long WriteFinishBytes { get; set; }

    public double Percent { get; set; }

    public DateTimeOffset UpdatedAt { get; set; } = DateTimeOffset.Now;

    public static GameInstallHistoryItem FromTask(GameInstallContext task)
    {
        return new GameInstallHistoryItem
        {
            Id = CreateId(task.GameId, task.Operation, task.InstallPath),
            GameBizValue = task.GameId.GameBiz.Value,
            ...
            DownloadTotalBytes = task.Progress_DownloadTotalBytes,
            DownloadFinishBytes = task.Progress_DownloadFinishBytes,
            WriteTotalBytes = task.Progress_WriteTotalBytes,
            WriteFinishBytes = task.Progress_WriteFinishBytes,
            Percent = task.Progress_Percent,
            UpdatedAt = DateTimeOffset.Now,
        };
    }

    public static string CreateId(GameId gameId, GameInstallOperation operation, string? installPath)
    {
        return $"{gameId.GameBiz}:{gameId.Id}:{operation}:{installPath}".ToLowerInvariant();
    }
}
