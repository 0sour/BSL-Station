...
internal class UrlProtocolService
{
    ...
    public static void RegisterProtocol()
    {
        ...
        {
            exe = AppConfig.StarwardPortableLauncherExecutePath ?? Path.Join(Path.GetDirectoryName(AppContext.BaseDirectory.TrimEnd('\\', '/')), "BSL-Station.exe");
        }
        else
        {
        ...
        Registry.SetValue(@"HKEY_CURRENT_USER\Software\Classes\Starward\DefaultIcon", "", $"{exe},1");
    ...
    public static async Task<bool> HandleUrlProtocolAsync(string url)
    {
        ...
        catch (Exception ex)
        {
            ...
            User32.MessageBox(HWND.NULL, ex.Message, AppConfig.ProductName);
            return true;
        }
        return false;
    }
