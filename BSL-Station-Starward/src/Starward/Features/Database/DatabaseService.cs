...
internal static class DatabaseService
{
    ...
    public static void SetDatabase(string folder)
    {
        ...
        {
            _databasePath = Path.GetFullPath(Path.Combine(folder, "BSL-Station.db"));
...
#pragma warning disable CS0162 // 检测到无法访问的代码
            string folder = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), @"BSL-Station\DatabaseBackup");
#pragma warning restore CS0162 // 检测到无法访问的代码
            Directory.CreateDirectory(folder);
            string file = Path.Combine(folder, $"BSL-Station_AutoBackup_{DateTime.Now:yyyyMMdd_HHmmss}.db");
            string archive = Path.ChangeExtension(file, ".7z");
            string archive_tmp = archive + "_tmp";
            string[] files = Directory.GetFiles(folder, "BSL-Station_AutoBackup_*.7z");
            if (files.Length == 0)
            {
                BackupDatabase(file);
