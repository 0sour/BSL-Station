...
public sealed partial class AppBackground : UserControl
{
    ...
    private readonly BslOfficialBackgroundService _bslOfficialBackgroundService = new();
    ...
    private string? _staticBackgroundOverrideUri;

    private string? _bslGameId;


    public GameId? CurrentGameId
    {
        get => _currentGameId;
        set => SetBackgroundContext(value, _staticBackgroundOverrideUri);
    }


    public void SetBackgroundContext(GameId? gameId, string? staticBackgroundOverrideUri = null, string? bslGameId = null)
    {
        bool gameChanged = _currentGameId != gameId;
        bool overrideChanged = !string.Equals(_staticBackgroundOverrideUri, staticBackgroundOverrideUri, StringComparison.Ordinal);
        bool bslChanged = !string.Equals(_bslGameId, bslGameId, StringComparison.Ordinal);

        _currentGameId = gameId;
        _staticBackgroundOverrideUri = staticBackgroundOverrideUri;
        _bslGameId = bslGameId;

        if (gameChanged || overrideChanged || bslChanged)
        {
            _ = UpdateBackgroundAsync();
        }
    }
    ...
    private void InitializeBackgroundImage()
    {
        try
        {
            if (CurrentGameId is GameId currentGameId)
            {
            ...
            if (!string.IsNullOrWhiteSpace(_staticBackgroundOverrideUri))
            {
                BackgroundImageSource = new BitmapImage(new Uri(_staticBackgroundOverrideUri));
            }
            else
            {
    ...
    public async Task UpdateBackgroundAsync(GameBackground? background = null)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(_bslGameId))
            {
                GameBackground? bslBackground = await _bslOfficialBackgroundService.TryGetBackgroundAsync(_bslGameId, cancellationToken);
                if (bslBackground is not null)
                {
                    DisposeVideoResource();
                    BackgroundImageSource = null;

                    string? filePath;
                    if (bslBackground.Type is GameBackground.BACKGROUND_TYPE_VIDEO && !string.IsNullOrWhiteSpace(bslBackground.Video?.Url))
                    {
                        filePath = await _backgroundService.GetBackgroundFileAsync(bslBackground.Video.Url, cancellationToken);
                        if (BackgroundService.FileIsSupportedVideo(filePath))
                        {
                            StartMediaPlayer(filePath);
                            _ = ChangeAccentColorToImageFileAsync(bslBackground.Background.Url, cancellationToken);
                        }
                        else
                        {
                    ...
                    {
                        filePath = await _backgroundService.GetBackgroundFileAsync(bslBackground.Background.Url, cancellationToken);
                        imageFilePath = filePath;
                        await ChangeBackgroundImageAsync(filePath, cancellationToken);
                    }

                    _lastBackgroundFile = imageFilePath ?? bslBackground.Video?.Url ?? bslBackground.Background.Url;
                    _lastScale = this.XamlRoot is not null ? this.XamlRoot.GetUIScaleFactor() : 1;
                    CurrentGameBackground = bslBackground;
                    return;
                }
            }
            ...
            {
                DisposeVideoResource();
                if (_lastBackgroundFile != _staticBackgroundOverrideUri || BackgroundImageSource is null)
                {
                    BackgroundImageSource = new BitmapImage(new Uri(_staticBackgroundOverrideUri));
                }
                _lastBackgroundFile = _staticBackgroundOverrideUri;
                _lastScale = this.XamlRoot is not null ? this.XamlRoot.GetUIScaleFactor() : 1;
