...
public sealed partial class ScreenshotSetting : PageBase
{
    ...
    private async Task TestCaptureAsync()
    {
        try
        {
            ...
            Directory.CreateDirectory(Path.Combine(ScreenshotFolder, AppConfig.ProductName));
            ...
            filePath = Path.Join(ScreenshotFolder, AppConfig.ProductName, $"{AppConfig.ProductName}_{frameTime:yyyyMMdd_HHmmssff}.{extension}");
