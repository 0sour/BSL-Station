...
public static partial class AppConfig
{
    ...
    public static async Task CheckEnviromentAsync()
    {
        try
        {
            ...
            string portableExe = Path.Join(parentFolder, "BSL-Station.exe");
            ...
            {
                CacheFolder = Path.Combine(Path.GetPathRoot(AppContext.BaseDirectory)!, ".BSL-StationCache");
                ConfigPath = Path.Combine(CacheFolder, "config.ini");
            }
            else if (IsPortable)
            {
                CacheFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), ProductFolderName);
                ConfigPath = Path.Combine(parentFolder!, "config.ini");
            }
            else
            {
            ...
            {
#if DEBUG
                using RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\BSL-Station.Debug");
#else
                using RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\BSL-Station");
#endif
            ...
            {
                userDataFolder = GetDefaultUserDataFolder();
                ...
                if (string.IsNullOrWhiteSpace(ConfigPath))
                {
                    ...
                    using RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\BSL-Station");
#endif
                    key.SetValue("UserDataFolder", userDataFolder);
                }
            }
        ...
        {
            User32.MessageBox(HWND.NULL, $"{Lang.AppConfig_AnUnknownIssueOccurredDuringInitialization}\n{ex.Message}", ProductName, User32.MB_FLAGS.MB_OK);
            Environment.Exit(0);
        }
    }


    private static string GetDefaultUserDataFolder()
    {
        string? parentFolder = new DirectoryInfo(AppContext.BaseDirectory).Parent?.FullName;
        if (IsPortable && !string.IsNullOrWhiteSpace(parentFolder))
        {
            return Path.Combine(parentFolder, "Data");
        }
        if (IsAppInRemovableStorage)
        {
            return Path.Combine(Path.GetPathRoot(AppContext.BaseDirectory)!, ".BSL-StationData");
        }
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), ProductFolderName, "Data");
    }
    ...
    {
#if DEBUG
        using var key = Registry.CurrentUser.CreateSubKey(@"Software\BSL-Station.Debug");
#else
        using var key = Registry.CurrentUser.CreateSubKey(@"Software\BSL-Station");
#endif
