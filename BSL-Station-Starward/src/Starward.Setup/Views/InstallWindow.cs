...
public class InstallWindow : WindowBase
{
    ...
    private void SetDefaultInstallFolder()
    {
        try
        {
            ...
            if (!Directory.Exists(folder) || !string.Equals(new DirectoryInfo(folder).Name, Branding.ProductFolderName, StringComparison.OrdinalIgnoreCase))
            {
                folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), Branding.ProductFolderName);
            }
            SetInstallFolder(folder);
        }
        catch
        {
            SetInstallFolder(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), Branding.ProductFolderName));
        }
    }
    ...
    private async void ChangeInstallFolder()
    {
        try
        {
            ...
            {
                if (!string.Equals(new DirectoryInfo(folder).Name, Branding.ProductFolderName, StringComparison.OrdinalIgnoreCase))
                {
                    folder = Path.Combine(folder, Branding.ProductFolderName);
                }
                SetInstallFolder(folder);
    ...
    private async void StartInstall()
    {
        ...
        try
        {
            if (!string.Equals(new DirectoryInfo(InstallFolder.Value).Name, Branding.ProductFolderName, StringComparison.OrdinalIgnoreCase))
            {
                if (SilentMode)
                {
            ...
            await File.WriteAllTextAsync(Path.Combine(InstallFolder.Value, Branding.InstallMarkerFileName), InstallFolder.Value);
            CreateShortcut();
    ...
    private void Launch()
    {
        try
        {
            string exe = Path.Combine(InstallFolder.Value, Branding.MainExeName);
            if (File.Exists(exe))
            {
                Process.Start("explorer", $"""
                    "{Path.Combine(InstallFolder.Value, Branding.MainExeName)}"
                    """);
                Environment.Exit(0);
            }
            else
    ...
    {
        string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), Branding.ProductName);
        Directory.CreateDirectory(folder);
        CreateShortcutInternal(Path.Combine(folder, Branding.ShortcutFileName));
        try
        {
            // 创建桌面快捷方式可能被安全软件拦截
            CreateShortcutInternal(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), Branding.ShortcutFileName));
        }
        catch { }
    }
    ...
    private void CreateShortcutInternal(string shortcutPath)
    {
        ...
        {
            Path = Path.Combine(InstallFolder.Value, Branding.MainExeName),
