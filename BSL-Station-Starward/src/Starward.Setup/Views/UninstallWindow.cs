...
public class UninstallWindow : WindowBase
{
    ...
    private async void CheckUninstall()
    {
        try
        {
            ...
            string flag = Path.Combine(currentFolder!, Branding.InstallMarkerFileName);
            ...
            bool hasDatabase = files.Any(f => Path.GetFileName(f).Equals(Branding.DatabaseFileName, StringComparison.OrdinalIgnoreCase));
            if (hasDatabase)
            {
                caution = true;
    ...
    private async void Uninstall()
    {
        try
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    string cacheFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), Branding.ProductFolderName);
    ...
    {
        string lnk = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), Branding.ShortcutFileName);
        if (File.Exists(lnk))
        {
            File.Delete(lnk);
        }
