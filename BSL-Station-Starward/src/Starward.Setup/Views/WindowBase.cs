...
public class WindowBase : Window
{
    ...
    protected async Task<bool> CheckProcessAsync(string? baseFolder = null)
    {
        ...
        string processName = Path.GetFileNameWithoutExtension(Branding.MainExeName);
        var ps = Process.GetProcessesByName(processName).Where(p => p.Id != Environment.ProcessId).Where(x => x.MainModule?.FileName.StartsWith(baseFolder ?? "") ?? true);
        if (ps.Any())
        {
