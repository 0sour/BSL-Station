...
public class DownloadService
{
    ...
    {
        if (Path.Combine(installFolder, Branding.SetupExeName) != Environment.ProcessPath)
        {
            Directory.CreateDirectory(installFolder);
            File.Copy(Environment.ProcessPath!, Path.Combine(installFolder, Branding.SetupExeName), true);
        }
    }
